...
const fetchOrder = async (id) => {
    ...
    try {
        ...
        // Remove # from ID if present
        const cleanId = String(id).replace('#', '');
        const response = await fetch(`http://localhost:3001/api/orders/${cleanId}`);
                            ...
                            {order.status === 'shipped' && (
                                <button
                                    onClick={async () => {
                                        if (confirm('Konfirmasi barang sudah diterima?')) {
                                            try {
                                                const res = await fetch(`http://localhost:3001/api/orders/${order.id}/receive`, { method: 'PATCH' });
                                                if (res.ok) {
                                                    fetchOrder(order.id);
                                                }
                                            } catch (err) {
                        ...
                        <div className="mb-10 bg-white p-6 rounded-2xl shadow-sm border border-orange-100 relative overflow-hidden">
                            ...
                            <button
                                onClick={async () => {
                                    ...
                                    try {
                                        const res = await fetch(`http://localhost:3001/api/orders/${order.id}/rating`, {
                                            method: 'POST',
